...
// close popup for regiset
    ...
    $("input[type='submit']").on("click", function (e) {
        ...
        var playername = $("input[placeholder='Enter Your Nick Name']").val()
        var playtime = $("input[placeholder='Enter Play Time in Seconds']").val()
        var breaktime = $("input[placeholder='Enter Break Time in Seconds']").val()
        var letterPattern=/^[A-Za-z]+$/;
        if(!letterPattern.test(playername)){
            alert('Name should only contain letters.');
            event.preventDefault();
            return;
        }
        
        var data_ofUser = [playername,level,playtime,breaktime];
